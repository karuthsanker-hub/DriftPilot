...
| [docs/REFACTOR_PLAN_V3_CATALYST_LAYER.md](REFACTOR_PLAN_V3_CATALYST_LAYER.md) | 🟡 PAUSED | Catalyst-driven selection layer plan. v1 spike returned DEAD on within-minute analysis (methodology artifact). v2b spike at daily granularity returned MARGINAL: news-days are 1.085× more volatile in the mean and 1.39× more likely to produce >2% moves on mega-caps. Signal IS real but too weak as a sole filter; doc has refined recommendations and three follow-up sub-spikes (mid-cap test, news+RVOL combo, curated-news subset) any of which could revive the v3 build. |
