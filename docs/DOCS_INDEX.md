...
| [docs/REFACTOR_PLAN_V3_CATALYST_LAYER.md](REFACTOR_PLAN_V3_CATALYST_LAYER.md) | 🟡 PAUSED | Catalyst-driven selection layer plan. Three spikes ran: v1 (within-minute, DEAD — methodology artifact), v2 (daily granularity, MARGINAL 1.085× on mega-caps), v3 (categorized by stock × news_category × subcategory — surfaces clear ANTI-SIGNALS in product_launch / analyst_target_raise / lawsuit, but no actionable strong signal on the thin Q1 sample). Refined recommendation: re-run categorized spike on broader universe + longer window before any infrastructure work. |
