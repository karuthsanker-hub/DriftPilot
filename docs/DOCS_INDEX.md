...
| [docs/REFACTOR_PLAN_V3_CATALYST_LAYER.md](REFACTOR_PLAN_V3_CATALYST_LAYER.md) | 🟢 ACTIONABLE-VALIDATED | Catalyst-driven selection layer plan. Five spikes — v1-v3 dead/marginal, v4 first actionable, **v5 validated 2026-05-03 on 50 mid-caps × full 2024 (510 events): `earnings/report` 5.09× @ 60m N=33, `analyst/target_cut` 2.91× @ 240m N=33, `analyst/target_raise` 1.42× @ 60m N=104**. Build order locked: event bus → earnings_report_v1 → target_cut as negative filter → target_raise long signal → universe-filter wiring for the existing 4 technical signals. |
| [docs/V3_AGENT_PLAN.md](V3_AGENT_PLAN.md) | ✅ CURRENT | Token-conscious agent execution plan for v3 build: 4 waves, 9 agents total + 4 reviewers (~200k tokens end-to-end). Each agent prompt is self-contained and includes explicit file lists to bound context. Reviewer-gate between waves; abort condition if classifier diverges from spike. |
| [docs/PORTFOLIO_CONTROLLER_DESIGN.md](PORTFOLIO_CONTROLLER_DESIGN.md) | 🟡 DESIGN-ONLY | Portfolio-controller layer above the slots — the "fund manager" intervention point. Protocol with 4 decision methods (score_candidate, allocate, evaluate_exit_override, should_halt). Two implementations to build: RuleBasedPortfolioController (default, fast, deterministic) and LlmPortfolioController (Qwen-driven, bounded multipliers). Backtest A/B framework defined. **DO NOT BUILD YET** — gated on 2-3 weeks of v3 paper-trading evidence. |
| [reports/COMPARISON.md](../reports/COMPARISON.md) | 🟢 V3 BREAKTHROUGH | **earnings_report_v1 + Qwen positive sentiment = PASS (edge_ratio 1.62)**. The catalyst spike measured absolute return magnitude; the long-only strategy needs directional moves. Adding Qwen-sentiment as a gate flipped the verdict from FAIL to PASS. v1 batch (4 technical signals) all FAIL on raw universe — v3 layered selection is the working fix. target_raise didn't move (consensus already priced in); v3.1 hypothesis is surprise-vs-consensus instead of polarity. |
